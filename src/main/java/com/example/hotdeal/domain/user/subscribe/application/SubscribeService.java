...
public class SubscribeService {
	...
	public List<SubscribeResponse> subscribeProduct(Long userId, List<Long> productIds) {
		...
		return savedSubscribes.stream()
			.map(sub -> new SubscribeResponse(userId, sub.getProductId()))
	...
	public List<SubscribeResponse> getSubscribeUserByProductId(Long productId) {
		...
		return subscribes.stream()
			.map(subscribe -> new SubscribeResponse(subscribe.getUserId(), subscribe.getProductId()))
			.collect(Collectors.toList());
	}
